var obj = {
	...
	dialogKg: document.querySelector('#kg'),
...
function cardOpen() {
	obj.dialogBg.style.display = 'flex';
	...
	if (itemTitle.innerHTML.indexOf('Трюфель') == -1) {
		obj.dialogKg.innerHTML = 'шт';
	} else {
		obj.dialogKg.innerHTML = 'кг';
	}
	...
	obj.orderBtn.onclick = function addToBasket() {
		...
		if (basketData.hasOwnProperty(itemId)) { // если такой товар уже есть в корзине - добавляем +1 к его количеству
			...
			basketData[itemId] = [itemImg, itemTitle, itemQuantity, itemPrice, itemId, itemKg];
		}
